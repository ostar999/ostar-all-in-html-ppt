...
(function () {
  ...
  ready(function () {
    ...
    function loadScript(src){return new Promise(function(ok,rej){
      ...
      s.onload=ok;s.onerror=function(){rej(new Error('CDN load fail: '+src))};
    ...
    async function getJSZip(){
      ...
      _jszip=window.JSZip;return _jszip;
    }

    function buildExportDialog(){
      if(exportDialog)return;
      exportDialog=document.createElement('div');exportDialog.className='export-overlay';
      exportDialog.innerHTML='<div class="export-panel">'+
      ...
      window.addEventListener('resize',function(){
        ...
        requestAnimationFrame(function(){
          var thumbs=exportDialog.querySelectorAll('.export-thumb');
          if(!thumbs.length)return;
          var scale=thumbs[0].clientWidth/1920;
          exportDialog.querySelectorAll('.mini-slide').forEach(function(m){m.style.transform='scale('+scale+')'});
        });
      });
    }
    ...
    function renderExportThumbs(){
      ...
      slides.forEach(function(s,i){
        ...
        var inner=document.createElement('div');inner.className='export-thumb-inner';
        var mini=document.createElement('div');mini.className='mini-slide';
        var clone=s.cloneNode(true);
        clone.classList.add('is-active');clone.style.position='absolute';clone.style.inset='0';
        clone.style.transform='none';clone.style.opacity='1';clone.style.padding='72px 96px';
        mini.appendChild(clone);inner.appendChild(mini);
        var check=document.createElement('div');check.className='export-check';
        ...
        var label=document.createElement('div');label.className='export-thumb-label';
        label.innerHTML='<span class="export-thumb-num">'+(i+1)+'</span><span class="export-thumb-title">'+_escapeHTML(title).slice(0,60)+'</span>';
        t.appendChild(inner);t.appendChild(check);t.appendChild(label);
        t.addEventListener('click',function(){t.classList.toggle('selected');updateExportCount()});
        grid.appendChild(t);
      });
      requestAnimationFrame(function(){
        var thumbs=grid.querySelectorAll('.export-thumb');
        if(!thumbs.length)return;
        var scale=thumbs[0].clientWidth/1920;
        grid.querySelectorAll('.mini-slide').forEach(function(m){m.style.transform='scale('+scale+')'});
      });
      updateExportCount();
    }
    ...
    function getSelectedExportIndices(){
      var r=[];
      exportDialog.querySelectorAll('.export-thumb.selected').forEach(function(t){r.push(parseInt(t.getAttribute('data-idx')))});
      return r.sort(function(a,b){return a-b});
    }

    function toggleExportDialog(force){
      if(!exportDialog)buildExportDialog();
      if(force===undefined&&_exporting)return;
      var open=force!==undefined?force:!exportDialog.classList.contains('open');
      exportDialog.classList.toggle('open',open);
      if(open){
        renderExportThumbs();
        if(overview&&overview.classList.contains('open'))overview.classList.remove('open');
      }
    }

    function _getAllCSS_sync(){
      var parts=[];
      document.querySelectorAll('style').forEach(function(s){parts.push(s.textContent)});
      return parts.join('\n');
    }

    async function _getAllCSS(){
      var parts=[];
      document.querySelectorAll('style').forEach(function(s){parts.push(s.textContent)});
      var links=document.querySelectorAll('link[rel="stylesheet"]');
      for(var i=0;i<links.length;i++){
        try{var res=await fetch(links[i].href);if(res.ok)parts.push(await res.text())}catch(e){}
      }
      return parts.join('\n');
    }

    function _buildRootBlock(){
      var rootCS=window.getComputedStyle(document.documentElement);
      var vars=[];
      for(var i=0;i<rootCS.length;i++){
        var prop=rootCS.item(i);
        if(prop.startsWith('--'))vars.push(prop+': '+rootCS.getPropertyValue(prop)+';');
      }
      return ':root {\n'+vars.join('\n')+'\n}';
    }
    ...
    function _showExportProgress(pct,text){
      var bar=exportDialog.querySelector('#exp-progress');bar.classList.add('active');
      exportDialog.querySelector('#exp-bar-fill').style.width=pct+'%';
      exportDialog.querySelector('#exp-progress-text').textContent=text;
    }
    function _hideExportProgress(){
      exportDialog.querySelector('#exp-progress').classList.remove('active');
      exportDialog.querySelector('#exp-bar-fill').style.width='0%';
    }
    function _disableExportBtns(v){
    ...
    async function doExport(mode){
      ...
      try{
        if(mode==='pdf'){_showExportProgress(10,'准备打印...');await exportToPDF(indices)}
          ...
          await exportToSVGs(indices,css);
        }
    ...
    async function exportToSVGs(indices,css){
      ...
      for(var j=0;j<total_;j++){
        var pct=5+Math.round((j/total_)*90);
        _showExportProgress(pct,'生成第 '+(j+1)+'/'+total_+' 个 SVG...');
        var idx=indices[j];
        var svg=slideToSVG(slides[idx]);
        var escapedCSS=_escapeXML(css);
        svg=svg.replace(/(<style><!\[CDATA\[)([\s\S]*?)(\]\]><\/style>)/,function(m,open,c1,close){return open+escapedCSS+'\n/* === inline === */\n'+c1+close});
        var title=slides[idx].getAttribute('data-title')||('slide-'+(idx+1));
        var fname='slide-'+(idx+1)+'-'+title.replace(/[^a-zA-Z0-9一-鿿_-]/g,'_').slice(0,40)+'.svg';
        zip.file(fname,svg);
  ...
  timerCount.textContent = (idx + 1) + ' / ' + total;
    ...
    document.addEventListener('keydown', function (e) {
      ...
      switch (e.key) {
        ...
        case 'p': case 'P': toggleExportDialog(); break;
        case 'Escape': toggleOverview(false); toggleNotes(false); toggleExportDialog(false); break;
      }
    });
