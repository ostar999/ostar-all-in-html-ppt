...
> 提词器 + 计时器），以及**内置 PDF/SVG/PNG 一键导出**。纯静态 HTML/CSS/JS，
> 无需构建。
...
> 一行命令装好 **36 主题 × 20 Canvas FX × 31 布局 × 15 完整 deck + 演讲者模式 + PDF/SVG/PNG 导出**。
...
- **📥 导出 PDF**：调用浏览器原生打印（`@page{size:1920px 1080px}` 精确 16:9），Chrome 中选择「存储为 PDF」→ 横向 → 保存
...
> PDF 使用浏览器原生渲染引擎，SVG 可单独在浏览器中编辑。PNG 走 `data:` URL（而非 `blob:` URL）避免 canvas 跨域污染，完美保留所有样式、颜色和渐变（字体可能降级为系统字体）。如需完全保真的 PNG，可使用 `scripts/render.sh`（headless Chrome）。
...
| 📄 **PDF/SVG/PNG 导出** | **新增** | `P` 键 / `runtime.js` |
