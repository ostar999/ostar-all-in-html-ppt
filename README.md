...
> 提词器 + 计时器），以及**内置 PDF(SVG)/PDF(PNG)/SVG/PNG 一键导出**。纯静态 HTML/CSS/JS，
> 无需构建。

🚀 基于 [lewislulu/html-ppt-skill](https://github.com/lewislulu/html-ppt-skill) Fork 改进优化，感谢作者 lewis 做出的杰出贡献。新增 P 键 PDF/SVG/PNG 一键导出功能等功能。
**English docs:** [README.EN.md](README.EN.md)

![html-ppt 封面 · 实时预览](docs/readme/hero.gif)

> 一行命令装好 **36 主题 × 20 Canvas FX × 31 布局 × 15 完整 deck + 演讲者模式 + PDF(SVG)/PDF(PNG)/SVG/PNG 导出**。
...
在任何 deck 里按 `P` 键，弹出**导出对话框**，以缩略图网格展示所有幻灯片：

- **勾选/取消**：点击缩略图切换选中状态，支持**全选 / 取消全选**
- **📥 导出 PDF (SVG)**：浏览器原生打印 DOM（文字可选、可搜索、矢量），文件名 `_svg.pdf`。Chrome 中选择「存储为 PDF」→ 横向 → 保存
- **📥 导出 PDF (PNG)**：先将每页渲染为 PNG 图片再合并打印（光栅、无字体依赖、跨平台一致），文件名 `_png.pdf`。适合分享给没有安装对应字体的用户
- **📦 导出 SVG (.zip)**：每页生成独立 SVG 文件（`foreignObject` + 全量 CSS），JSZip 打包下载
- **🖼️ 导出 PNG (.zip)**：通过 SVG → data URL → canvas 管线渲染为像素级 PNG。单页直出 `.png`，多页自动打包 `.zip`

> PDF (SVG) 使用浏览器原生渲染引擎，文字可选可搜索；PDF (PNG) 先渲染为图片再打印，跨平台显示完全一致。PNG 走 `data:` URL（而非 `blob:` URL）避免 canvas 跨域污染，完美保留所有样式、颜色和渐变。如需完全保真的 PNG，可使用 `scripts/render.sh`（headless Chrome）。
...
| 📄 **PDF/SVG/PNG 导出** | **新增双模式** | `P` 键 / `runtime.js` |
...
F                               全屏
S                               打开演讲者窗口（磁吸卡片模式）
P                               导出对话框 — 选页 → PDF(SVG) / PDF(PNG) / SVG(.zip) / PNG(.zip)
N                               底部 notes 抽屉
R                               重置计时器（演讲者窗口内）
O                               slide 总览网格
T                               切换主题（自动同步到演讲者窗口）
